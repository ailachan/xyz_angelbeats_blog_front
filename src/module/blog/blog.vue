<template>
  <div>
      ...
      <div class="ui container">
          ...
          <div class="ui grid">
            <div class="eleven wide column">
              <div class="ui horizontal link list" style="text-align: center !important;">
                <div class="item">
                  <img :src="blogAuthorInfo.user_avatar" alt="" class="ui avatar image">
                  <div class="content">{{ blogAuthorInfo.user_name }}</div>
        ...
        <div class="ui attached positive message">
          <div class="ui middle aligned grid">
            <div class="eleven wide column">
              <ul class="list">
                <li>作者：<span>{{ blogAuthorInfo.user_name }}</span></li>
    ...
    <slot class="ui bottom aligned footer_position"></slot>
  </div>
  ...
  methods: {
    findBlog() {
      this.$http.get(this.$base_url + "/blogs/" + this.$route.query.id).then(res => {
        ...
        this.findComment();
        this.findBlogAuthor(this.blog.user_id)
      })
    },
    findComment() {
    ...
    findBlogAuthor(user_id) {
      this.$http.get(this.$base_url + "/blogs/selectUserById/" + user_id).then(res => {
        this.blogAuthorInfo = res.data.data
      })
    },
    postComment() {
      ...
      if (this.comment && this.comment.blog_id && this.comment.comment_content) {
        ...
        if (localStorage.getItem('token')){//如果管理员用户已登录则使用登录用户的头像和昵称
          this.comment.commit_avatar = localStorage.getItem('avatar')
          this.comment.comment_nickname = localStorage.getItem('userName')
        }
